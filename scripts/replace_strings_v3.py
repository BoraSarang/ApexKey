#!/usr/bin/env python3
"""
Text("...") → Text(LocalizedStringKey("key")) 치환기 v3
- LocalizedStringKey 사용 (SwiftUI 동적 키 지원)
"""
import re
import hashlib
...
ko_content = ko_path.read_text(encoding="utf-16")
ko_map = {}
for m in re.finditer(r'"([^"]+)"\s*=\s*"([^"]+)";', ko_content):
    ko_map[m.group(2)] = m.group(1)
...
for f in SRC_ROOT.rglob("*.swift"):
    ...
    def replacer(match):
        full = match.group(0)
        value = match.group(1)
        if ".localized" in full or "LocalizedStringKey" in full:
            return full
        if "{" in value or "%" in value or "\\(" in value:
            return full
        key = ko_map.get(value)
        if key:
            return f'Text(LocalizedStringKey("{key}"))'
        return full
