...
enum ActionCategory: String, Codable, CaseIterable, Identifiable {
    ...
    var displayName: String {
        switch self {
        case .essential:    return "category.essential".localized
        case .scripting:    return "category.scripting".localized
        case .media:        return "category.media".localized
        case .documents:    return "category.documents".localized
        case .location:     return "category.location".localized
        case .content:      return "category.content".localized
        case .accessories:  return "category.accessories".localized
        case .flowControl:  return "category.flowControl".localized
        case .variables:    return "ui.variables".localized
        case .ai:           return "Apple Intelligence"
        case .appIntents:   return "category.appIntents".localized
        case .automation:   return "category.automation".localized
        }
    }
    ...
    var actionTypes: [ActionType] {
        ...
        case .flowControl:  return [.runShortcut, .repeatLoop, .repeatEach, .ifElse, .endRepeat, .stopShortcut, .chooseFromMenu, .comment, .runScriptInShell]
        ...
        case .automation:   return [.automation, .findAutomation, .automationRun, .trigger]
        }
    }
}
...
enum ActionType: String, Codable, CaseIterable, Identifiable {
    ...
    var category: ActionCategory {
        Self.metadata[self]?.category ?? .essential // 누락 시 기본값 (테스트가 전수 보장)
    }

    var displayName: String {
        Self.metadata[self]?.displayName ?? rawValue
    }

    var systemImage: String {
        Self.metadata[self]?.systemImage ?? "questionmark"
    }
}